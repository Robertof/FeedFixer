...
FeedFixer::XML::FeedPP -- Parse/write/merge/edit RSS/RDF/Atom syndication feeds
...
Get an RSS file and parse it:
    ...
    my $feed = FeedFixer::XML::FeedPP->new( $source );
...
Generate an RDF file and save it:
    ...
    my $feed = FeedFixer::XML::FeedPP::RDF->new();
    $feed->title( "use Perl" );
...
Convert some RSS/RDF files to Atom format:
    ...
    my $feed = FeedFixer::XML::FeedPP::Atom::Atom10->new();        # create empty atom file
...
C<FeedFixer::XML::FeedPP> is an all-purpose syndication utility that parses and
...
module except for FeedFixer::XML::TreePP.

=head1 METHODS FOR FEED

=head2  $feed = FeedFixer::XML::FeedPP->new( "index.rss" );

This constructor method creates an C<FeedFixer::XML::FeedPP> feed instance. The only
...
=head2  $feed = FeedFixer::XML::FeedPP->new( "http://use.perl.org/index.rss" );

The URL on the remote web server is also available as the first argument.
L<LWP::UserAgent> is required to download it.

=head2  $feed = FeedFixer::XML::FeedPP->new( '<rss version="2.0">...' );

The XML source code is also available as the first argument.

=head2  $feed = FeedFixer::XML::FeedPP->new( $source, -type => $type );

The C<-type> argument allows you to specify type of $source
from choice of C<'file'>, C<'url'> or C<'string'>.

=head2  $feed = FeedFixer::XML::FeedPP->new( $source, utf8_flag => 1 );

This makes utf8 flag on for every feed elements.
Perl 5.8.1 or later is required to use this.

Note that any other options for C<FeedFixer::XML::TreePP> constructor are also
allowed like this. See more detail on L<FeedFixer::XML::TreePP>.

=head2  $feed = FeedFixer::XML::FeedPP::RSS->new( $source );
...
=head2  $feed = FeedFixer::XML::FeedPP::RDF->new( $source );
...
=head2  $feed = FeedFixer::XML::FeedPP::Atom->new( $source );
...
Atom 1.0 feed is also supported since C<FeedFixer::XML::FeedPP> version 0.30.
Atom 0.3 is still default, however, future version of this module
would create Atom 1.0 as default.

=head2  $feed = FeedFixer::XML::FeedPP::Atom::Atom03->new();

This creates an empty Atom 0.3 instance obviously.

=head2  $feed = FeedFixer::XML::FeedPP::Atom::Atom10->new();

This creates an empty Atom 1.0 instance intended.

=head2  $feed = FeedFixer::XML::FeedPP::RSS->new( link => $link, title => $tile, ... );
...
$srcitem is a C<FeedFixer::XML::FeedPP::*::Item> class's instance
...
C<FeedFixer::XML::FeedPP> allows you to describe date/time using any of the three
following formats:
...
setter method to manipulate C<media:content> element, etc.
    ...
    my $feed = FeedFixer::XML::FeedPP::RSS->new();
...
C<FeedFixer::XML::FeedPP> requires only L<FeedFixer::XML::TreePP>
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP;
use strict;
use Carp;
use Time::Local;
use FeedFixer::XML::TreePP;
...
sub new {
    my $package = shift;
    my( $init, $source, @rest ) = &FeedFixer::XML::FeedPP::Util::param_even_odd(@_);
    ...
    if ( exists $self->{rss} ) {
        FeedFixer::XML::FeedPP::RSS->feed_bless($self);
    }
    elsif ( exists $self->{'rdf:RDF'} ) {
        FeedFixer::XML::FeedPP::RDF->feed_bless($self);
    }
    elsif ( exists $self->{feed} ) {
        my $xmlns = $self->{feed}->{-xmlns} if exists $self->{feed}->{-xmlns};
        if ( $xmlns eq $XMLNS_ATOM10 ) {
            FeedFixer::XML::FeedPP::Atom::Atom10->feed_bless($self);
        }
        elsif ( $xmlns eq $XMLNS_ATOM03 ) {
            FeedFixer::XML::FeedPP::Atom::Atom03->feed_bless($self);
        }
        else {
            FeedFixer::XML::FeedPP::Atom->feed_bless($self);
        }
    }
    else {
...
sub load {
    ...
    my $tpp = FeedFixer::XML::TreePP->new(%$TREEPP_OPTIONS, %$opts);
...
sub to_string {
    my $self = shift;
    my( $args, $encode, @rest ) = FeedFixer::XML::FeedPP::Util::param_even_odd(@_);
    $args ||= \@rest;
    my @opts = ( output_encoding => $encode ) if $encode;
    my $tpp = FeedFixer::XML::TreePP->new( %$TREEPP_OPTIONS, @opts, @$args );
...
sub merge {
    my $self   = shift;
    my $source = shift;
    my $target = ref $source ? $source : FeedFixer::XML::FeedPP->new($source);
...
sub merge_common_channel {
    ...
    my $xmlchk = { map { $_ => 1 } @xmlns1, @$FeedFixer::XML::FeedPP::XMLNS_NOCOPY };
...
sub add_clone_item {
    ...
    if ( ref $dstitem eq ref $srcitem ) {
        FeedFixer::XML::FeedPP::Util::merge_hash( $dstitem, $srcitem );
    }
    else {
...
sub get_pubDate_w3cdtf {
    my $self = shift;
    my $date = $self->get_pubDate_native();
    FeedFixer::XML::FeedPP::Util::get_w3cdtf($date);
}

sub get_pubDate_rfc1123 {
    my $self = shift;
    my $date = $self->get_pubDate_native();
    FeedFixer::XML::FeedPP::Util::get_rfc1123($date);
}

sub get_pubDate_epoch {
    my $self = shift;
    my $date = $self->get_pubDate_native();
    FeedFixer::XML::FeedPP::Util::get_epoch($date);
}
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Plugin;
use strict;
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Item;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Element );

*get_pubDate_w3cdtf  = \&FeedFixer::XML::FeedPP::get_pubDate_w3cdtf;   # import
*get_pubDate_rfc1123 = \&FeedFixer::XML::FeedPP::get_pubDate_rfc1123;
*get_pubDate_epoch   = \&FeedFixer::XML::FeedPP::get_pubDate_epoch;

sub elements {
    my $self = shift;
    my $args = [ @_ ];
    my $methods = { map {$_=>1} @$FeedFixer::XML::FeedPP::ITEM_METHODS };
    while ( my $key = shift @$args ) {
        my $val = shift @$args;
        if ( $methods->{$key} ) {
            $self->$key( $val );
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::RSS;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP );
...
sub channel_class {
    'FeedFixer::XML::FeedPP::RSS::Channel';
}

sub item_class {
    'FeedFixer::XML::FeedPP::RSS::Item';
}
...
sub init_feed {
    ...
    $self->{rss}->{'-version'} ||= $FeedFixer::XML::FeedPP::RSS20_VERSION;
...
sub merge_native_channel {
    ...
    FeedFixer::XML::FeedPP::Util::merge_hash( $self->{rss}, $tree->{rss}, qw( channel ) );
    FeedFixer::XML::FeedPP::Util::merge_hash(
        $self->{rss}->{channel},
        $tree->{rss}->{channel},
        qw( item )
    );
...
sub add_item {
    my $self = shift;
    my( $init, $link, @rest ) = &FeedFixer::XML::FeedPP::Util::param_even_odd(@_);
    ...
    my $item = FeedFixer::XML::FeedPP::RSS::Item->new(@rest);
...
sub pubDate {
    ...
    $date = FeedFixer::XML::FeedPP::Util::get_rfc1123($date);
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::RSS::Channel;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Element );

# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::RSS::Item;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Item );
...
sub guid {
    ...
    if ( ! scalar @args ) {
        ...
        # FeedFixer::XML::FeedPP 0.36's behavior
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::RDF;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP );
...
sub channel_class {
    'FeedFixer::XML::FeedPP::RDF::Channel';
}

sub item_class {
    'FeedFixer::XML::FeedPP::RDF::Item';
}
...
sub init_feed {
    ...
    $self->xmlns( 'xmlns'     => $FeedFixer::XML::FeedPP::XMLNS_RSS );
    $self->xmlns( 'xmlns:rdf' => $FeedFixer::XML::FeedPP::XMLNS_RDF );
    $self->xmlns( 'xmlns:dc'  => $FeedFixer::XML::FeedPP::XMLNS_DC );
...
sub merge_native_channel {
    ...
    FeedFixer::XML::FeedPP::Util::merge_hash( $self->{'rdf:RDF'}, $tree->{'rdf:RDF'},
        qw( channel item ) );
    FeedFixer::XML::FeedPP::Util::merge_hash(
...
sub add_item {
    ...
    my $item = FeedFixer::XML::FeedPP::RDF::Item->new(@rest);
    $item->link($link) if $link;
    $item->elements(@$init) if ref $init;
...
sub __refresh_items {
    ...
    foreach my $item (@$list) {
        my $rdfli = FeedFixer::XML::FeedPP::Element->new();
...
sub pubDate {
    ...
    my $date = shift;
    return $self->get_pubDate_w3cdtf() unless defined $date;
    $date = FeedFixer::XML::FeedPP::Util::get_w3cdtf($date);
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::RDF::Channel;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Element );

# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::RDF::Item;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Item );
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom::Common;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP );
...
sub merge_native_channel {
    ...
    FeedFixer::XML::FeedPP::Util::merge_hash( $self->{feed}, $tree->{feed}, qw( entry ) );
}
...
sub image {
    ...
    my $rext = join( "|", map {"\Q$_\E"} keys %$FeedFixer::XML::FeedPP::MIME_TYPES );

    if ( defined $href ) {
        my $ext = ( $href =~ m#[^/]\.($rext)(\W|$)#i )[0];
        my $type = $FeedFixer::XML::FeedPP::MIME_TYPES->{$ext} if $ext;
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom::Atom03;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Atom::Common );

sub channel_class {
    'FeedFixer::XML::FeedPP::Atom::Atom03::Feed';
}

sub item_class {
    'FeedFixer::XML::FeedPP::Atom::Atom03::Entry';
}
...
sub init_feed {
    ...
    $self->xmlns( 'xmlns' => $FeedFixer::XML::FeedPP::XMLNS_ATOM03 );
    $self->{feed}->{'-version'} ||= $FeedFixer::XML::FeedPP::ATOM03_VERSION;
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom::Atom10;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Atom::Common );

sub channel_class {
    'FeedFixer::XML::FeedPP::Atom::Atom10::Feed';
}

sub item_class {
    'FeedFixer::XML::FeedPP::Atom::Atom10::Entry';
}
...
sub init_feed {
    ...
    $self->xmlns( 'xmlns' => $FeedFixer::XML::FeedPP::XMLNS_ATOM10 );
#   $self->{feed}->{'-version'} ||= $FeedFixer::XML::FeedPP::ATOM10_VERSION;
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Atom::Atom03 );

# @ISA = qw( FeedFixer::XML::FeedPP::Atom::Atom10 );   # if Atom 1.0 for default

# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom::Common::Feed;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Element );
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom::Atom03::Feed;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Atom::Common::Feed );

# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom::Atom10::Feed;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Atom::Common::Feed );

# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom::Common::Entry;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Item );
...
*_fetch_value = \&FeedFixer::XML::FeedPP::Atom::Common::Feed::_fetch_value;

# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom::Atom03::Entry;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Atom::Common::Entry );
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Atom::Atom10::Entry;
use strict;
use vars qw( @ISA );
@ISA = qw( FeedFixer::XML::FeedPP::Atom::Common::Entry );
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Element;
use strict;
...
# ----------------------------------------------------------------
package FeedFixer::XML::FeedPP::Util;
use strict;
