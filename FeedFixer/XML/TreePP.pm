...
FeedFixer::XML::TreePP -- Pure Perl implementation for parsing/writing XML documents
...
parse an XML document from file into hash tree:
    ...
    my $tpp = FeedFixer::XML::TreePP->new();
...
get a remote XML document by HTTP-POST and parse it into hash tree:
    ...
    use FeedFixer::XML::TreePP;
    my $tpp = FeedFixer::XML::TreePP->new( force_array => [qw( item )] );
...
FeedFixer::XML::TreePP module parses an XML document and expands it for a hash tree.
...
This constructor method returns a new FeedFixer::XML::TreePP object with C<%options>.

    $tpp = FeedFixer::XML::TreePP->new( %options );
